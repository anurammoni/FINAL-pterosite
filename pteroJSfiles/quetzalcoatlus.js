...
const renderer = new THREE.WebGLRenderer(
  {
    canvas: document.querySelector('#quetzal'), alpha: true
  });
...
const pteroTexture = new THREE.TextureLoader().load('pteroUV/quetzalcoatlusSkin.png');
const ptero = await loader.loadAsync('ptero3D/quetzalcoatlus.obj');
...
const pointLight = new THREE.PointLight(0xCDE6AC, 1350, 0);
pointLight.position.set(1,2,20);

const ambLight = new THREE.AmbientLight(0x6971CF, 5);
...
let targetRotX = 0, targetRotY = 0;
 ...
 targetRotY = mouseX * Math.PI * 0.2 - 1.3;  // left-right
  targetRotX = mouseY * Math.PI * 0.2 - 0.6;  // up-down
